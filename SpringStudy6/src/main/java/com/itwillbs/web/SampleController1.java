...
//	private static final Logger logger = LoggerFactory.getLogger(SampleController1.class);
	...
	// @RequestMapping(path ="매핑할 주소" , method = "전달 방식(get/post)",)
	
	// http://localhost:8088/web/doA 주소 입력
	
	// * 메서드의 리턴타입이 void일때 자동으로 (주소이름과 같은) jsp 페이지를 연결
	@RequestMapping(path ="/doA" , method = RequestMethod.GET)
	public void doA() {
		System.out.println(" doA() 메서드 실행 ");
		logger.info(" (●'◡'●) Logger doA() 메서드 실행 (●'◡'●) ");
		logger.debug("(●'◡'●) doA() 메서드 실행 (●'◡'●)");
		
	}
	
	// http://localhost:8088/web/doA2.me => doA2.me.jsp(x) doA2.jsp(o) 
	// doA2 주소로 doA2.jsp페이지를 연결
	@RequestMapping(path = "/doA2")
	public void doA2() {
	...
	// * 404페이지에 메세지가 있다 => 컨트롤러에 매핑 o , 연결된 뷰페이지(JSP)가 없음.
	
	
	// @RequestMapping(path = "/doA3")
	@GetMapping(value = "/doA3")
	public void doA3() {
		logger.debug("(●'◡'●) doA3() 메서드 실행 (●'◡'●)");
		logger.debug(" /views/doA3.jsp 페이지 실행");
	}
